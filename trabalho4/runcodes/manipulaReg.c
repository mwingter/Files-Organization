...
//	printf("Entrando em ordenaLista_atualizaEncAndTopoLista, n_rem: %d\n", n_rem);
	...
	//ordenando a lista de registros de removidos
	//MS_sort(lista, n_rem, sizeof(REG_REM), int_compare_tempo_reverse);
...
//	printf("removido = |%c|\n", rd->removido);
	...
	fwrite(&rd->removido, REM_TAM, 1, bin); //reescrevendo o removido de '*' para '-'
	//printf("Tam logo ANTES de passar pro arquivo = %d\n", rd->tamanhoRegistro);
	fwrite(&rd->tamanhoRegistro, TAM_TAM, 1, bin);

	//==== testando o tamanho escrito
	//int tam_teste = 0;
	//fseek(bin, -(TAM_TAM), SEEK_CUR);
	//fread(&tam_teste, TAM_TAM, 1, bin);
	//printf("teste TAM NO ARQUIVO = %d\n", tam_teste);
...
void atualiza_UmRegistro(REGDADOS* rd, char nomeCampoAtualiza[MAX], char valorCampoAtualiza[MAX]){
	...
	printf("REGISTRO ANTES DE ATUALIZAR\n");
	printaUmReg(rd);
	...
	printf("REGISTRO DEPOIS DE ATUALIZAR\n");
	printaUmReg(rd);
